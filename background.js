// Copyright (c) 2014 Joseph Mansfield.
// Adapted from Google Mail Checker, Copyright (c) 2012 The Chromium Authors
// Use of this source code is governed by the MIT license and the original
// BSD-style license which can be found in the LICENSE.md file.
...
var filters = {
  ...
  url: [{urlContains: getGmailUrl().replace(/^https?\:\/\//, '')},
        {urlContains: getInboxUrl().replace(/^https?\:\/\//, '')}]
};

function onNavigate(details) {
  if (details.url && (isGmailUrl(details.url) || isInboxUrl(details.url))) {
